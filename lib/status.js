...
function runPythonCmd(pythonExecutable, ...arguments){
  return new Promise((resolve) =>{
    try{
      const spawn = require('child_process').spawn;
      const python = spawn(pythonExecutable, arguments);
      const output = []

      python.on('error', (error) => {
        if(error.toString().indexOf('ENOENT')>=0){
          resolve({status: 'error', message: `Command '${pythonExecutable}' could not be found`})
        } else{
          resolve({status: 'error', message: (error.stack ? error.stack : error)})
        }
      });
      python.on('close', (code) => {
        if(code===0){
          resolve({status: 'success', output: output.join('').trim()})
        } else{
          resolve({status: 'error', message: `Command '${pythonExecutable}' returned exit code '${code}'`})
        }
      });
...
function getPythonInfo(pythonExecutable){
  return Promise.all([
    runPythonCmd(pythonExecutable, '-V'), 
    runPythonCmd(pythonExecutable, '-c', 'import sys; print(",".join(sys.path))')
  ])
  .then((values) => {
    const versionInfo = values[0]
    const moduleSearchPathInfo = values[1]
    let moduleSearchPath = []
    messages = []
    if(versionInfo.message){
      messages.push(versionInfo.message)
    }
    if(versionInfo.status==='success' && moduleSearchPathInfo.status==='success'){
      try{
        if(moduleSearchPathInfo.output){
          moduleSearchPath.push(...moduleSearchPathInfo.output.split(','))
        }
      } catch(error){
        messages.push(error.stack ? error.stack : error)
      }
    }
    return status = {
...
function buildOutput(resourceInfo, environmentInfo, pythonExecutableInfo){
  ...
  if(pythonExecutableInfo.status==='success'){
    ...
    if(pythonExecutableInfo.moduleSearchPath.length==0){
      output.push(tab(`Module search path: n/a`, 1))
    } else{
      output.push(tab(`Module search path:`, 1))
      pythonExecutableInfo.moduleSearchPath
      .filter(p => p.trim()!=='')
      .forEach(p => output.push(tab(p, 2)))
    }
  }
  
  if(pythonExecutableInfo.message){
    output.push(tab('Error:', 1))
...
module.exports = {
  ...
  renderStatus(resourcePath, settings){
    ...
    getPythonInfo(settings.pythonExecutable).then(pythonInfo =>{
      view.innerHTML = buildOutput(resourceInfo, environmentInfo, pythonInfo)
    })
