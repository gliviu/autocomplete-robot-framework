  ...
  try{
    if(!resourcePath){
      ...
      if(!currentResource){
        ...
        // Imported libraries
        let libraryNames = currentResource.imports.libraries.map(lib => lib.name)
        libraryNames = new Set(['BuiltIn', ...libraryNames])
        ...
        for(const importedResource of currentResource.imports.resources){
          ...
          if(importedResource.resourceKey){
              const resource = keywordsRepo.resourcesMap[importedResource.resourceKey]
              if(resource){
                resInfo.status = 'Resolved'
                resInfo.sourcePath = resource.path
              } else{
                resInfo.status = 'Error resolving resource'
              }
          } else{
            let resourcesByName = keywordsRepo.computeGroupedResources(true).resourcesByName.get(resourceName.toLowerCase()) || []
