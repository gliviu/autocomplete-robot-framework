...
var createMatchScore = function(match){
  ...
  for (var i = 1; i < match.length; i++) {
    score+=(match[i]?match[i].length:0);
  }
  return score;
}
...
/**
 * Given prefix 'seva' creates regex
 * (?:^|[ .])(?:s)(?=e|.*?[ .]e)(?:e|(.*?)[ .]e)(?=v|.*?[ .]v)(?:v|(.*?)[ .]v)(?=a|.*?[ .]a)(?:a|(.*?)[ .]a)
...
var createPrefixRegexp = function(prefix, settings) {
  ...
  if(prefix.length===1){
    ...
    for (var i = 0; i < prefix.length; i++) {
      ...
      if(i===0){
        // First char
        prefixRegexpArr.push(util.format('(?:%s)', escapeRegExp(prefix.charAt(i))));
        var nextChar = prefix.charAt(i+1);
        prefixRegexpArr.push(util.format('(?=%s|.*?[ .]%s)', nextChar, nextChar))
      } else if(i<prefix.length-1){
        // Between
        prefixRegexpArr.push(util.format('(?:%s|(.*?)[ .]%s)', currentChar, currentChar))
        var nextChar = prefix.charAt(i+1);
        prefixRegexpArr.push(util.format('(?=%s|.*?[ .]%s)', nextChar, nextChar))
      } else{
        // Last char
