  ...
  if (fileExists(path) && pathUtils.normalize(dirPath)!==PYTHON_LIBDOC_DIR) {
    ...
    if (isRobotFile(fileContent, path, settings)) {
      ...
      if (libraryInfoUpdated) {
        libManager.importLibraries(PYTHON_LIBDOC_DIR, PYTHON_EXE);
      }
...
var provider = {
  ...
  load() {
    ...
    atom.commands.add('atom-text-editor', 'Robot Framework:Show status', function() {
      editor = atom.workspace.getActiveTextEditor()
      if(editor){
        status.renderStatus(editor.getPath())
      }
    });
  },
