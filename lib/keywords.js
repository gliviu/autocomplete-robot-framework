...
var addKeywords = function(parsedRobotInfo, resourceKey, resourcePath, resourceName) {
 ...
 */
var getResourcesByName = function(){
  const resourcesByName = new Map()
  for(const resourceKey in resourcesMap){
    const resource = resourcesMap[resourceKey]
    let names = resourcesByName.get(resource.name)
    if(!names){
      names = []
      resourcesByName.set(resource.name, names)
    }
    names.push(resource)
  }
