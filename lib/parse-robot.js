...
var middleSectionRegexp = namedRegexp(/[\r\n]+ *([*]+ ?(:<sectionName>(Test cases)|(Keywords)|(Variables)|(Settings))( ?[*]*)?(  .*?)?(?=[\r\n]))/ig); // middle of the file
var firstSectionRegexp = namedRegexp(/^ *([*]+ ?(:<sectionName>(Test cases)|(Keywords)|(Variables)|(Settings))( ?[*]*)?(  .*?)?(?=[\r\n]))/i); // section that starts at offset 0
...
var getSections = function(fileContent){
    ...
    while(match = middleSectionRegexp.exec(fileContent)){
        sectionArray.push({
...
var parse = function(fileContentOrig) {
    ...
    var fileContentNoTabs = fileContentOrig.replace(tabRegexp, '  ');
    fileContent = fileContentNoTabs.replace(singleLineCommentRegexp, '');
    ...
    fillLineNumbers(fileContentNoTabs, keywords);
    return {
        keywords: keywords,
        testCases: testCases,
...
var fillLineNumbers = function(fileContentNoTabs, keywords){
    debugger
    ...
    var lines = fileContentNoTabs.split(/\r\n|\n|\r/);
