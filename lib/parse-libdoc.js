...
const parse = function(fileContent) {
  ...
  const keywords = [];
  const libdoc = parser.parse(fileContent, {
    attributeNamePrefix: '',
    ignoreAttributes: false
  })
  const libdocKeywords = libdoc.keywordspec.kw
  if(!libdocKeywords){
    return undefined
  }
  ...
  //   keywordName = replaceXmlSpecialCharacters(keywordName)
...
const xmlSpecialCharMap = new Map([
    [/&quot;/g, '"'],
    [/&lt;/g, '<'],
    [/&gt;/g, '>'],
    [/&amp;/g, '&'],
...
function replaceXmlSpecialCharacters(str) {
  for (const [key, value] of xmlSpecialCharMap) {
...
// todo remove
function main(){
  const f = fs.readFileSync('/home/liviu/tmp/00/BuiltIn-3.xml').toString()
  ...
  // const j = parser.parse(f, {
  //   attributeNamePrefix: '',
  //   ignoreAttributes: false
  // })
  // console.log(JSON.stringify(j.keywordspec.kw, null, 2))
}
