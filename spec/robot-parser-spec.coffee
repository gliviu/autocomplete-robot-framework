...
describe 'Robot parser', ->
  ...
  it 'correctly handle tabs and spaces', ->
    ...
    expect(robot.keywords[3]).toBeUndefined()
  describe 'Section parsing', ->
    ...
    it 'correctly interpret line ending', ->
      delim = '\n'
      robot = robotParser.parse("*test cases*#{delim}t1#{delim}*keywords*#{delim}k1#{delim}")
  ...
  describe 'Keyword parsing', ->
    it 'correctly interpret line ending', ->
      delim = '\n'
      robot = robotParser.parse("*keywords*#{delim}k1#{delim}k2#{delim}")
      expect(robot.keywords[0]?.name).toEqual 'k1'
